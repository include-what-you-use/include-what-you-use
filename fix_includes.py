...
class IWYUOutputRecord(object):
  ...
  def __init__(self, filename):
    ...
    # 'to-keep' #includes.  If flags.nocomments is True, the comments
  ...
  def ParseOneRecord(self, iwyu_output, flags):
    ...
    Arguments:
      ...
      flags: commandline flags, as parsed by optparse.  We use
         flags.nocomments, which controls whether we output comments
         generated by iwyu.
    Returns:
...
def _DecoratedMoveSpanLines(iwyu_record, file_lines, move_span_lines):
  ...
  (iwyu_record will never have any comments if flags.nocomments is
  True).
...
def main(argv):
  ...
  parser.add_option('-b', '--blank_lines', action='store_true',
