...
# for a while.
OVERLOADED_FUNCS = {'std::abs', 'std::acos', 'std::acosh', 'std::asin',
...
def process_synopsis(headername, syn, lang):
    p = Parser(syn)
    p.parse()
    for symbol in p.found_names:
...
def process_tex_file(tex_file, lang):
    for m in SYNOPSIS.finditer(tex_file):
        process_synopsis(m.group('headername'), m.group('code'), lang)
